...
class TeacherTimeTableController extends Controller
{
    public function index()
    {
        $entries = TeacherTimeTable::with(['level', 'classroom', 'subject', 'teacher.user'])
            ...
            ->orderBy('start_time')
            ->get()
            ->map(function (TeacherTimeTable $row) {
                return [
                    'id' => $row->id,
                    'day' => $row->day,
    ...
    public function store(Request $request)
    {
        ...
        return response()->json([
            'data' => [
                ...
                'level_name' => $entry->level->name ?? null,
                'class_id' => $entry->class_id,
                'class_name' => $entry->classroom->name ?? null,
                'subject_id' => $entry->subject_id,
                'subject_name' => $entry->subject->name ?? null,
                'teacher_id' => $entry->teacher_id,
                'teacher_name' => $entry->teacher->full_name ?? null,
                'teacher_email' => optional($entry->teacher->user)->email,
            ],
        ], 201);
    }
}
